...
public class AddHearts extends BaseCommand {
  ...
  public void onAddHearts(final CommandSender sender, final OnlinePlayer target, final int hearts) {
    ...
    final Component addHeartsMsg = MiniMessage.miniMessage().deserialize(this.lifeSteal.getKey("Messages.AddHearts"),
            ...
            playerName);
    this.lifeSteal.getInteraction().verbose(Logger.Level.INFO, addHeartsMsg, player,
    ...
    this.IStatisticsManager.setCurrentHearts(player, this.IStatisticsManager.getCurrentHearts(player) + hearts)
            .setPeakReachedHearts(player, this.IStatisticsManager.getPeakReachedHearts(player) + hearts)
            .update(player);
  }
}
